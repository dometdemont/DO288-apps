...
CLOUD=(openstack openstack openstack)
...
FIPAPI=(10.33.102.217 10.33.102.218 10.33.102.228)
FIPAPP=(10.33.102.146 10.33.102.248 10.33.102.139)
WORKERS=(2 0 3)
MASTERS=(3 3 3)
...
metadata:
  name: sanity
  annotations:
    description: 'Three OpenShift clusters running a basic set of backing services : jade, green and sinople, the first attached to its own UPF 5.0.0-522 and UPF router ubuntu 16.04'
objects:
  - kind: ServiceAccount
    apiVersion: v1
    metadata:
...
for _project in $oc_projects ; do if [[ "$_project" =~ ^openshift.* ]] ; then _newproject_prefix="adm" ; else _newproject_prefix="" ; fi ; oc project $_project &>> $logfile || oc $_newproject_prefix new-project $_project --display-name="Project $_project" --description='From HPE5g automated deployer 2021-06-21 Version 0.97 on: Mon Jun 21 2021 15:08:50 GMT+0200 (Central European Summer Time) Three OpenShift clusters running a basic set of backing services : jade, green and sinople, the first attached to its own UPF 5.0.0-522 and UPF router ubuntu 16.04' &>> $logfile || _fail_ "Cannot find or create project $_project missing" ; done
